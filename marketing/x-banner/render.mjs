...
const html = path.join(here, "sts-x-banner-600x1800.html");
const url = "file://" + html;
...
await page.pdf({
  path: path.join(here, "sts-x-banner-600x1800.pdf"),
  width: `${W_MM}mm`,
  height: `${H_MM}mm`,
...
await page150.screenshot({ path: path.join(here, "sts-x-banner-600x1800@150dpi.png"), fullPage: false });
...
await pagePrev.screenshot({ path: path.join(here, "preview.png"), fullPage: false });

await browser.close();

for (const f of ["sts-x-banner-600x1800.pdf", "sts-x-banner-600x1800@150dpi.png", "preview.png"]) {
