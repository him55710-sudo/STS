...
- **콘텐츠는 전부 실사 사진, 상품은 전부 실존 제품입니다.** 일러스트/가상 브랜드 시드 데이터는 모두 제거했습니다.
- **룩 게시물 10종**: 남성 룩 5종(post-look1~5, @minu.archive) + 여성 룩 5종(post-look6~10, @edit.eunseo · @rin.heritage).
  프레피 / 브리티시 헤리티지 / 미니멀 컨템포러리 / 아웃도어 시티 / 프렌치 럭셔리.
- **구매 링크**: 정확한 상품명 검색 결과(네이버 쇼핑) 딥링크 — 판매처 홈이 아니라 상품 카드에 바로 도달합니다.
- **동일/유사 구분**: 브랜드·모델이 확인된 것만 "동일 상품"이고, 특정할 수 없는 오브젝트(주얼리, 일부 시계·팬츠)는 "유사 상품"으로 연결합니다.
- **실루엣**: 시드 게시물의 오브젝트 폴리곤은 fashion_v2 온디바이스 세그멘테이션으로 생성한 실측 마스크입니다(초소형 주얼리 3건은 bbox 폴백).
- 시드 애널리틱스 수치와 팔로워 수는 UI 시연용 가정값입니다.
