...
import { isAdpickConfigured, searchAdpickProducts } from "@/lib/affiliate/adpick";
...
export async function POST(req: NextRequest) {
  ...
  const [adpick, naver, llm] = await Promise.all([
    isAdpickConfigured() ? searchAdpickProducts(queries[0] ?? "") : Promise.resolve([]),
  ...
  const adpickCandidates: WebCandidate[] = adpick.map((item, index) => ({
    id: `adpick-${index}`,
    brand: null,
    ...
    url: item.commissionUrl ?? nv(item.title),
    imageUrls: item.imageUrl ? [item.imageUrl] : [],
    source: item.source,
    pageTrust: 0.9,
    affiliate: Boolean(item.commissionUrl),
    commissionRate: item.commissionRate ?? undefined,
  }));
  const seenNames = new Set(adpickCandidates.map((c) => norm(c.productName)));
  naver.forEach((c) => seenNames.add(norm(c.productName)));
  ...
  if (candidates.length === 0) {
    return NextResponse.json({
      candidates: [],
      provider: isAdpickConfigured() || isNaverConfigured() ? "empty" : "none",
    });
  }
  ...
  return NextResponse.json({
    candidates,
    provider: candidates[0]?.source ?? "none",
    adpickCount: adpickCandidates.length,
