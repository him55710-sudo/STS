...
export default function CreatorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const seedCreator = CREATORS.find((c) => c.id === id);
  const isRemote = isUuid(id);

  // 서버 프로필 + 팔로워 수 — 피드에 아직 없는 크리에이터의 프로필도 열린다
  useEffect(() => {
    if (!isRemote) return;
    let cancelled = false;
    fetchCreatorProfile(id).then((r) => {
      if (!cancelled && r) setFetched(r.creator);
    });
    return () => {
      cancelled = true;
    };
  ...
  const creator = seedCreator ?? fetched ?? (isRemote ? lookupCreator(id) : null);

  if (!creator) {
    const loading = isRemote && isBackendConfigured() && !remoteLoaded;
    return (
      <div className="px-4 py-16 text-center text-sm text-ink-2">
        {loading ? "불러오는 중..." : "크리에이터를 찾을 수 없어요."}
      </div>
    );
  }

  const posts = [
    ...remotePosts.filter((p) => p.creatorId === id),
    ...(seedCreator && isDemoMode() ? POSTS.filter((p) => p.creatorId === id) : []),
  ];
  const shopProducts = [
    ...new Set(posts.flatMap((p) => p.objects.map((o) => o.productId)).filter((x) => x != null)),
  ]
    .map((pid) => lookupProduct(pid))
    .filter((p) => p != null);
  ...
  return (
    <div>
      ...
      {/* 하이라이트 (인스타 스타일) */}
      {creator.avatarImage && posts.length > 0 && (
      ...
      {tab === "posts" ? (
        <div className="grid grid-cols-3 gap-0.5 pt-0.5">
          ...
          {posts.length === 0 && (
            <p className="col-span-3 py-12 text-center text-sm text-ink-2">아직 게시물이 없어요.</p>
          )}
        </div>
      ) : (
        <div className="p-3">
