...
## Cycle 16 — webkr 등록 후 실측: 품질 게이트 2종 추가
- **Plan**: 사용자가 API HUB에 웹문서(webkr)를 추가. 실제로 쓸 만한 상품이 나오는지 검증.
- **Check (프로덕션 실측)**: `registered:true, contract:apihub, 200, total 563,171`.
  등록은 성공. 그러나 **확보한 2건이 둘 다 불량**이었다:
  1. `"( 개월 기준) - 바이모노"` (bymono.com/product/detail.html) — 할부 안내 문구가
     페이지 제목으로 잡힘. 상품명이 아니다. ← 이번 사이클에서 내가 넣은
     허용목록 완화(PRODUCT_PATH_RE)가 통과시킨 건. 완화의 실제 비용이 드러났다.
  2. `"크림색 남자 니트"` (musinsa.com/search/goods?keyword=…) — **검색 결과 페이지**를
     상품으로 제시. 크리에이터가 특정 상품을 태그할 수 없다.
- **Do**: 품질 게이트 2종.
  - `LISTING_URL_RE`: /search·/list·/categor·/ranking·?keyword= 등 목록·검색 페이지 배제
  - `isUsableProductName()`: 의미 글자 6자 이상 + **검색어 토큰 최소 1개 겹침**
    (이미지 제목 필터에서 효과가 검증된 규칙을 webkr에 동일 적용)
  - 실측 5케이스 단위 검증 5/5 통과 (불량 2건 배제, 정상 상품 URL 3건 통과)
- **판단(정직 보고)**: webkr은 쇼핑 인덱스가 아니라 웹문서 인덱스다. 5건 중 몰 페이지
  2건, 게이트 통과 후 0건. **webkr의 기여는 부수적이고 이미지 제목 폴백이 주력**이다.
  등록이 무의미하진 않지만(브랜드명이 특정된 질의에서 직링크 확보 가능),
  "네이버 쇼핑몰 상품 DB 연동"의 대체재는 아니다.
- **Act**: 실제 상품 DB가 필요하면 제휴 네트워크 API(쿠팡 파트너스 등 사용자 계정 필요)가
  정공법. 앱의 제휴 수익 모델과도 일치. 사용자 결정 필요 사항으로 보고.
